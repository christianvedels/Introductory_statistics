...
- .red[**Next lecture:** used to build confidence intervals for $\mu$ when $\sigma^2$ is unknown]
]
...
- But $b^2$ is **biased**: $E(b^2) = \dfrac{n-1}{n}\sigma^2 < \sigma^2$ — it underestimates $\sigma^2$
]

---
# Why is $b^2$ biased? (Proof)

.pull-left-wide[
Decompose $(X_i - \bar{X}) = (X_i - \mu) - (\bar{X} - \mu)$, square and sum:
...
.pull-left-wide[
Take expectations (using $E(X_i-\mu)^2 = \sigma^2$ and $E(\bar{X}-\mu)^2 = \sigma^2/n$):
...
.pull-left-wide[
Therefore:
$$E(b^2) = \frac{1}{n}(n-1)\sigma^2 = \frac{n-1}{n}\sigma^2 < \sigma^2 \quad \blacksquare$$
...
.pull-left-wide[
- Dividing by $n-1$ instead of $n$ inflates the estimate just enough to remove the bias
...
---
# Why is $S$ biased? (Proof)

.pull-left-wide[
Since $\sqrt{\cdot}$ is a **concave** function, Jensen's inequality gives:
$$E\!\left[\sqrt{S^2}\right] \leq \sqrt{E\!\left[S^2\right]}$$
]

--

.pull-left-wide[
Since $S^2$ is unbiased, $E[S^2] = \sigma^2$, so:
$$E(S) \leq \sqrt{\sigma^2} = \sigma$$
- $S$ strictly underestimates $\sigma$ for finite $n$ $\quad\blacksquare$
]

--

.pull-left-wide[
As $n \to \infty$, $S^2 \xrightarrow{p} \sigma^2$, so $S \xrightarrow{p} \sigma$ — $S$ is **consistent** despite the bias.
]
...
---
# Using the chi-square: example

.pull-left-wide[
**Why do we care?** Knowing the distribution of $S^2$ lets us say how surprising an observed variance is — and later, build confidence intervals and hypothesis tests for $\sigma^2$.
]

--

.pull-left-wide[
**Example:** Wages are normally distributed with $\sigma^2 = 100$. We draw $n = 11$ observations and observe $S^2 = 130$. How likely is it to see a sample variance this large or smaller?
]

--

.pull-left-wide[
Scale to $\chi^2$: $\quad Y = S^2 \cdot \dfrac{n-1}{\sigma^2} = 130 \cdot \dfrac{10}{100} = 13$
...
.pull-left[
- Raw moments depend on the unit of measurement — we want scale-independent measures
...
- Testing whether a sample comes from a specific distribution (e.g. normality tests)
- Comparing two empirical distributions — are wages in 2000 and 2020 from the same distribution?
- Non-parametric inference: confidence bands for the full CDF without assuming a parametric form
]
