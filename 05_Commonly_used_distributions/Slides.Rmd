...
(c) Find $P(3 \leq Y \leq 9)$.
]

```{r practice-5, eval=FALSE, include=FALSE}
# ANSWER:
# (a) Z = (Y - 5) / 2, so Z ~ N(0,1)
# (b) P(Y <= 7) = Phi((7-5)/2) = Phi(1) ≈ 0.841
# (c) P(3 <= Y <= 9) = Phi((9-5)/2) - Phi((3-5)/2)
#                    = Phi(2) - Phi(-1)
#                    = Phi(2) - (1 - Phi(1))
#                    ≈ 0.977 - 0.159 = 0.818
```
...
.pull-left-wide[
**Q1.** $Y \sim \mathcal{N}(3, 9)$. What is $P(Y = 3)$?
...
---
# Example note: the Normal distribution

.pull-left-wide[
**What it models:** any continuous quantity that arises as the sum of many small independent influences — heights, measurement errors, estimators, test statistics

**Parameters:** $\mu$ (mean, shifts the centre) and $\sigma^2$ (variance, controls spread)

**Key facts:**
$$Y \sim \mathcal{N}(\mu, \sigma^2) \implies E(Y) = \mu, \quad Var(Y) = \sigma^2$$
$$Z = \frac{Y - \mu}{\sigma} \sim \mathcal{N}(0,1) \quad \text{(standardisation)}$$
$$P(Y \leq a) = \Phi\!\left(\frac{a-\mu}{\sigma}\right), \qquad \Phi(-z) = 1 - \Phi(z)$$

**Relationships:** $Bin(n,p) \approx \mathcal{N}(np,\, np(1-p))$ for large $n$ (CLT); OLS estimators are exactly normal when errors are normal, approximately normal otherwise
