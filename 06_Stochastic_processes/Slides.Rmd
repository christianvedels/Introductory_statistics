...
---
# Example: Election day 2026

.pull-left-wide[
- Poll (March 21): Red bloc 47.5%, Blue bloc 45.8%, Middle 6.6%
- For each bloc, define an indicator: $R_i = \mathbf{1}[\text{voter } i \text{ votes red}]$, and similarly $B_i$, $M_i$
]

--

.pull-left-wide[
- We poll 1000 voters: The **vote totals** after $n = 1{,}000$ voters are partial sum transformations:
$$N_R = \sum_{i=1}^{n} R_i \qquad N_B = \sum_{i=1}^{n} B_i \qquad N_M = \sum_{i=1}^{n} M_i$$
]

---
# Example: Election day 2026 (cont.)

.pull-left-wide[
- Each $R_i \sim \text{Bernoulli}(p)$, so $E(R_i) = p$ and $\text{Var}(R_i) = p(1-p)$
- For $n$ independent voters the expected totals and variances are:

| Bloc   | $p$   | $E(N)$  | $\text{Var}(N)$ | $\text{SD}(N)$ | $E(N) \pm 2\,\text{SD}$ |
|--------|-------|---------|-----------------|----------------|--------------------------|
| Red    | 0.475 | **475** | 249.4           | **15.8**       | [443, 507]               |
| Blue   | 0.458 | **458** | 248.2           | **15.8**       | [426, 490]               |
| Middle | 0.066 | **66**  | 61.6            | **7.8**        | [50, 82]                 |
...
- **A)** Binomial — because it counts events over time
- **B)** Geometric — special case of negative binomial with $x_0 = 1$
- **C)** Poisson — because arrivals are random
]
...
#   A: Binomial counts events over fixed time — confuses counting distribution with waiting distribution
#   B: Correct — Geo(p) = Negbin(1, p)
#   C: Poisson applies to continuous-time processes — confusing discrete vs continuous setting
```
...
- **C)** Binomial(10, 0.5) — because each of the 10 customers may return
]
...
#   B: Adds past count to the rate — confuses cumulative state with the rate parameter
#   C: Imports binomial logic into a continuous-time process
```
...
- Next time: Estimating the mean $\rightarrow$ Chapter 8
