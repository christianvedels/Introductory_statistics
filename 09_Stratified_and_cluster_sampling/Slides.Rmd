...
2. Calculate $\bar{X}_{st}$
3. Is this sample proportionally allocated? What does that imply for the simple average?
]

```{r practice1-answer, eval=FALSE, include=FALSE}
# ANSWER
# 1. w_Sales = 500/1000 = 0.5, w_IT = 300/1000 = 0.3, w_Finance = 200/1000 = 0.2
# 2. X_st = 0.5*40 + 0.3*50 + 0.2*60 = 20 + 15 + 12 = 47
# 3. n_j/n = N_j/N for all j (10/20=0.5, 6/20=0.3, 4/20=0.2): proportional allocation
#    With proportional allocation the simple average equals the stratified average:
#    simple = (10*40 + 6*50 + 4*60)/20 = (400+300+240)/20 = 940/20 = 47
```
...
.pull-left-wide[
- If $X$ and $Y$ are **independent**, stratifying makes no difference — $Var(\bar{X}_{st}) = Var(\bar{X})$
]

--

.pull-left-wide[
- If $X$ and $Y$ are **correlated**, within-stratum variance is smaller than the overall variance
...
.pull-left-wide[
**Q1.** When is $\bar{X}_{st}$ more efficient than $\bar{X}$?

- **A)** When $X$ and $Y$ are correlated, so within-stratum variance is lower than overall variance
- **B)** Always — stratification can only reduce variance, never increase it
- **C)** Only when proportional allocation is used
]

--

.pull-left-wide[
**Q2.** Why is $\bar{X}$ biased in a non-proportional stratified sample, but $\bar{X}_{st}$ is not?

- **A)** Because $\bar{X}$ uses the wrong formula for the stratum means
- **B)** Because $\bar{X}_{st}$ corrects for over- or under-representation via the weights $w_j$
- **C)** Because $\bar{X}$ is only unbiased in simple random samples drawn with replacement
]
...
#
# Q1: Answer A
#   A: Correct — correlation between X and Y means within-stratum variance < total variance,
#      so the weighted sum of stratum variances is smaller than the overall variance
#   B: Tempting — stratification sounds like it can only help; but if X and Y are
#      independent, Var(X_st) = Var(X_bar); efficiency depends on correlation, not just
#      the act of stratifying
#   C: Tempting — proportional allocation is a nice special case; but efficiency gains
#      depend on correlation between X and Y, not on the allocation rule per se
#
# Q2: Answer B
#   A: Tempting — students might think each stratum mean is estimated wrongly;
#      but X_bar_j is an unbiased estimator for mu_j within its stratum
#   B: Correct — the weights w_j = N_j/N restore the correct population proportions,
#      compensating for the non-representative sample structure
#   C: Tempting — replacement vs. non-replacement affects variance, not bias;
#      the issue here is weighting, not the sampling mechanism
```
...
- To improve on $\bar{X}$, we need to choose $n_j$ to minimise $Var(\bar{X}_{st})$
]
...
$$
\bar{X}_{st, prop} = \frac{1}{n} \cdot \sum_{j=1}^m \sum_{i=1}^{n_j} X_{ij}
$$
]
...
.pull-left-wide[
- This does **not** affect unbiasedness or consistency of $\bar{X}_{st}$
- But it **does** change the variance formula
]
...
.pull-left-wide[
**Strata** are **homogeneous** — all elements share the same value of $Y$
- Purpose: minimise within-group variance of $X$
...
.pull-left-wide[
**Clusters** are **heterogeneous** — each cluster should look like a mini-version of the full population
- Purpose: reduce cost
- We sample *some* clusters, not all
]

--

.pull-left-wide[
- Cluster sampling is motivated by **cost**, not statistical efficiency (unlike stratification)
- Lower cost means we can potentially afford a larger $n$
]
