...
.pull-right-wide[
```{r fig-normal-ci, eval=FALSE, include=FALSE}
library(ggplot2)

z_crit <- 1.96
x_grid <- seq(-3.5, 3.5, length.out = 2000)
...
dat$region <- ifelse(
  dat$x < -z_crit, "left_tail",
  ifelse(dat$x > z_crit, "right_tail", "middle")
)

p <- ggplot(dat, aes(x = x, y = y)) +
  geom_ribbon(
    aes(ymin = 0, ymax = y, fill = region, group = region),
    alpha = 0.55
  ) +
  ...
  geom_vline(
    xintercept = c(-z_crit, z_crit),
    linetype = "dashed",
    linewidth = 0.6
  ) +
  annotate("text", x = 0, y = 0.17, label = "1 - alpha",
           parse = TRUE, size = 5.5) +
  annotate("text", x = -2.7, y = 0.04, label = "alpha/2",
           parse = TRUE, size = 4, colour = "tomato3") +
  annotate("text", x = 2.7, y = 0.04, label = "alpha/2",
           parse = TRUE, size = 4, colour = "tomato3") +
  scale_x_continuous(
...
ggsave("Statistics/11_Confidence_intervals/Figures/normal-ci.png", plot = p, width = 6, height = 4)
```
...
.pull-right-wide[
```{r fig-chisq-ci, include=FALSE, eval=FALSE}
library(ggplot2)
...
x_grid <- seq(0.001, qchisq(0.999, nu), length.out = 2000)
...
dat_chi$region <- ifelse(
  dat_chi$x < lo, "left_tail",
  ifelse(dat_chi$x > hi, "right_tail", "middle")
)

p <- ggplot(dat_chi, aes(x = x, y = y)) +
  geom_ribbon(
    aes(ymin = 0, ymax = y, fill = region, group = region),
    alpha = 0.55
  ) +
  ...
  annotate("text", x = (lo + hi) / 2, y = 0.038,
           label = "1 - alpha", parse = TRUE, size = 5) +
  annotate("text", x = lo / 2, y = 0.010,
           label = "alpha/2", parse = TRUE, size = 3.8,
           colour = "tomato3") +
  annotate("text", x = hi + 3.5, y = 0.005,
           label = "alpha/2", parse = TRUE, size = 3.8,
           colour = "tomato3") +
  scale_x_continuous(
    breaks = c(lo, hi),
    labels = c(
      expression(chi[alpha/2]^2*(n-1)),
      expression(chi[1-alpha/2]^2*(n-1))
    )
  ) +
  labs(
    x = NULL,
    y = "Density",
    title = expression(chi^2(10)~"— right-skewed, asymmetric tails")
  ) +
  theme_minimal(base_size = 13) +
  theme(panel.grid.minor = element_blank())

ggsave("Statistics/11_Confidence_intervals/Figures/chisq-ci.png", plot = p, width = 6, height = 4)
```
...
```{r f-fig-save, eval=FALSE, include=FALSE}
library(ggplot2)
df_f <- data.frame(
  x = rep(seq(0.01, 5, length.out = 500), 4),
  d1 = rep(c(2, 5, 10, 20), each = 500),
  d2 = 20
)
df_f$density <- df(df_f$x, df1 = df_f$d1, df2 = df_f$d2)
df_f$dist <- factor(df_f$d1, labels = c("F(2,20)", "F(5,20)", "F(10,20)", "F(20,20)"))

p <- ggplot(df_f, aes(x = x, y = density, colour = dist)) +
...
ggsave("Statistics/11_Confidence_intervals/Figures/f-fig.png", p, width = 5, height = 3, dpi = 150)
```
...
.pull-right-wide[
```{r fig-f-ci, include=FALSE, eval=FALSE}
library(ggplot2)
...
x_grid <- seq(0.001, qf(0.999, d1, d2), length.out = 2000)
...
dat_f$region <- ifelse(
  dat_f$x < lo_f, "left_tail",
  ifelse(dat_f$x > hi_f, "right_tail", "middle")
)

p <- ggplot(dat_f, aes(x = x, y = y)) +
  geom_ribbon(
    aes(ymin = 0, ymax = y, fill = region, group = region),
    alpha = 0.55
  ) +
  ...
  geom_vline(
    xintercept = c(lo_f, hi_f),
    linetype = "dashed",
    linewidth = 0.6
  ) +
  annotate("text", x = (lo_f + hi_f) / 2, y = 0.25,
           label = "1 - alpha", parse = TRUE, size = 5) +
  annotate("text", x = lo_f / 2, y = 0.04,
           label = "alpha/2", parse = TRUE, size = 3.8,
           colour = "tomato3") +
  annotate("text", x = hi_f + 0.8, y = 0.03,
           label = "alpha/2", parse = TRUE, size = 3.8,
           colour = "tomato3") +
  scale_x_continuous(
    breaks = c(lo_f, hi_f),
  ...
  labs(
    x = NULL,
    y = "Density",
    title = "F(10, 15) — right-skewed, asymmetric tails"
  ) +
  theme_minimal(base_size = 13) +
  theme(panel.grid.minor = element_blank())

ggsave("Statistics/11_Confidence_intervals/Figures/f-ci.png", plot = p, width = 6, height = 4)
```
...
.pull-left-wide[
- The approximate CI for $p$ relies on the normal approximation to the binomial
...
.pull-left-wide[
**Example:** $n = 20$, $k = 2$ successes (so $\hat{p} = 0.10$)
...
.pull-left-wide[
- The **Clopper-Pearson interval** is constructed directly from the binomial distribution — no normal approximation
...
.pull-left-wide[
- The interval is **conservative**: the true coverage is at least $1-\alpha$ for every $p$ and $n$
...
.pull-left-wide[
**Setting:** $n = 20$ households surveyed, $k = 2$ report unemployment; $\hat{p} = 0.10$
